var ContinuousVisualization = function(height, width, context) {
	...
	this.draw = function(objects) {
		for (var i in objects) {
			var p = objects[i];
			if (p.Shape == "rect")
				this.drawRectange(p.x, p.y, p.w, p.h, p.Color, p.Filled, p.Label);
			if (p.Shape == "circle")
				this.drawCircle(p.x, p.y, p.r, p.Color, p.Filled, p.Label);
		};

	};

	this.drawCircle = function(x, y, radius, color, fill, label) {
		var cx = x * width;
		var cy = y * height;
		var r = radius;
		...
		if (fill) {
			...
			context.fill();
                ...
                if (color == "green") {
                    ...
                    context.fillText(label, cx, cy - 5);
                }
